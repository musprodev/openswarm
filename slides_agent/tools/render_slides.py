...
def convert_to_pdf(
    ...
    if not run_cmd_no_check(cmd_pdf, timeout_seconds=SOFFICE_TIMEOUT_SECONDS):
        return ""
    ...
    if not run_cmd_no_check(cmd_odp, timeout_seconds=SOFFICE_TIMEOUT_SECONDS):
        return ""
    ...
    if exists(odp_path):
        ...
        if not run_cmd_no_check(cmd_odp_pdf, timeout_seconds=SOFFICE_TIMEOUT_SECONDS):
            return ""
        if exists(pdf_path):
            return pdf_path
