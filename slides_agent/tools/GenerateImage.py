...
class GenerateImage(BaseTool):
    ...
    width: int = Field(default=1024, description="Image width in pixels (default 1024)")
    height: int = Field(default=1024, description="Image height in pixels (default 1024)")
    ...
    def _generate_with_gemini(self, prompt: str, model: str) -> bytes:
        ...
        try:
            ...
            raise RuntimeError(
                "Google GenAI package not installed. Please install: pip install google-genai"
            ) from e
        except Exception as e:
