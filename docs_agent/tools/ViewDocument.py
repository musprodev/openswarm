...
class ViewDocument(BaseTool):
    ...
    document_name: str = Field(..., description="Name of the document to view (without extension)")
    ...
    def run(self):
        ...
        try:
            ...
            source_size = (
                source_path.stat().st_size if source_path.exists() else md_path.stat().st_size
            )
...
<body>
    ...
    tool = ViewDocument(project_name="test_view", document_name="sample_doc", view_range=[1, 10])
    print(tool.run())
