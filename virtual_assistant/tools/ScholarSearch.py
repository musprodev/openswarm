...
class ScholarSearch(BaseTool):
    ...
    year_from: int | None = Field(
        default=None, description="Filter results from this year onwards (e.g., 2020)"
    )

    year_to: int | None = Field(
        default=None, description="Filter results up to this year (e.g., 2024)"
    )

    num_results: int = Field(
        default=10, ge=1, le=20, description="Number of results to return (1-20)"
    )
    ...
    def run(self):
        ...
        try:
            ...
            if not api_key:
                raise ValueError(
                    "SEARCH_API_KEY is not set. Add it to your .env to use ScholarSearch."
                )
            ...
            result = {
                "query": self.query,
                "filters": {"year_from": self.year_from, "year_to": self.year_to},
...
if __name__ == "__main__":
    ...
    print("-" * 60)
    tool = ScholarSearch(query="transformer architecture deep learning", num_results=5)
    result = tool.run()
