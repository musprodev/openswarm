...
class ProductSearch(BaseTool):
    ...
    country: str | None = Field(
        default="us", description="Country code (e.g., 'us', 'gb', 'de', 'fr', 'jp')"
    )

    language: str | None = Field(
        default="en", description="Interface language (e.g., 'en', 'es', 'fr', 'de')"
    )

    sort_by: (
        Literal["relevance", "review_score", "price_low_to_high", "price_high_to_low"] | None
    ) = Field(
        default="relevance",
        description="Sort results by: 'relevance', 'review_score', 'price_low_to_high', or 'price_high_to_low'",
    )

    price_min: float | None = Field(default=None, description="Minimum price filter")

    price_max: float | None = Field(default=None, description="Maximum price filter")

    condition: Literal["new", "used"] | None = Field(
        default=None, description="Filter by product condition: 'new' or 'used'"
    )

    num_results: int = Field(
        default=10, ge=1, le=60, description="Number of results to return (1-60)"
    )
    ...
    def run(self):
        ...
        try:
            ...
            if not api_key:
                raise ValueError(
                    "SEARCH_API_KEY is not set. Add it to your .env to use ProductSearch."
                )
...
if __name__ == "__main__":
    ...
    print("-" * 60)
    tool = ProductSearch(query="wireless headphones", num_results=5)
    result = tool.run()
